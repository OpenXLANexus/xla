...
ENTRY main {
  ...
  indices = s32[7,2,5] parameter(1)
  ROOT gather = s32[7,3,2,5] gather(operand, indices),
      offset_dims={1},
      collapsed_slice_dims={2},
      start_index_map={2},
      index_vector_dim=3,
  ...
  //CHECK: %[[BD2_DIV:.*]] = s32[] divide(s32[] %[[I]], s32[] %[[CONSTANT5]])
  //CHECK: %[[CONSTANT2:.*]] = s32[] constant(2)
  //CHECK: %[[BD1_DIV:.*]] = s32[] divide(s32[] %[[BD2_DIV]], s32[] %[[CONSTANT2]])
  //CHECK:  %[[BD0:.*]] = s32[1] broadcast(s32[] %[[BD1_DIV]])
  ...
  //CHECK: %[[BD2_RAW:.*]] = s32[] remainder(s32[] %[[I]], s32[] %[[CONSTANT5]])
